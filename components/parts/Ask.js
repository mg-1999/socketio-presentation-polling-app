    ...
    addChoiceButton(choice, i) {
      ...
      return (
        <div className="button-spacing col-xs-12 col-md-6">
          <button key = {i} className = {"btn btn-" + buttonTypes[i]} onClick = {this.select.bind(null, choice)}> {choice.toUpperCase()}. {this.props.question[choice]} </button>
        </div>);
    }
    , /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
       RENDERER
      ...
      return (
        <div id="currentQuestion">
          <Display if={this.state.answer}>
            <h4>Uw antwoord: {this.state.answer.toUpperCase()}. {this.props.question[this.state.answer]}</h4>
          </Display>
          <Display if = {!this.state.answer}> 
            <h4> {this.props.question.q} </h4>
            <div className="col-md-9"> {this.state.choices.map(this.addChoiceButton)} </div>
          </Display>
        </div>);
    }
});
